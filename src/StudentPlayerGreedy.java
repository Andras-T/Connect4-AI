...
public class StudentPlayerGreedy extends Player {
    private int enemyPlayer, DEPTH = 1;
    private boolean draw = false;

    public StudentPlayerGreedy(int playerIndex, int[] boardSize, int nToConnect) {
            ...
            for (int row = nCheck - 1; row < 6; row++) {
                for (int d = 0; d < nCheck; d++) {
                    if (state[row - d][col + d] == playerIndex) {
                        nInADiagonal++;
                        if (nInADiagonal == nCheck) {
                            if ((row - d - 1 >= 0 && col + d + 1 < 7 && state[row - d - 1][col + d + 1] == 0) || (row - d + nCheck < 6 && col + d - nCheck > 0 && state[row - d + nCheck][col + d - nCheck] == 0)) {
                                value += 3 * pow(10, nCheck);
                            }
                        }
                    } else {
